...
##Базовая настройка
1. Для работы скрипта необходимо установить: **python python-yaml python-paramiko git**  
В Debian это можно сделать так:  
	`apt-get install python python-yaml python-paramiko git`
...
Например:  
	...
	`192.168.1.254:22`  
	`192.168.2.254:8022`  
Пустые строки и комментарии в этом файле не допускаются!

5. Запускаем скрипт командой:  
	`python backup.py`
